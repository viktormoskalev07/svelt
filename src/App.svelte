...
let style
      ...
      if (!is_marketing) {
        ...
        if(staticWidget){
          ...
          if(script_url.searchParams.get('close-widget-static')){
            const staticStatus = script_url.searchParams.get('close-widget-static')
  ...
  selectShowVideo.removeAttribute("controls")   
    ...
    selectShowVideo.addEventListener('timeupdate', function (ev) {
      ...
      if(Math.ceil(ev.target.duration - (ev.target.currentTime - ev.target.currentTime / 60),10,)===1){
        index--
        if(index ===0){
        currentSeconds="00"
        }
    ...
    selectShowVideo.addEventListener('timeupdate', (event) => {
      ...
      if (selectShowVideo !== null && selectShowVideo !== undefined) {
        let barLength = (selectShowVideo.currentTime / selectShowVideo.duration) * 100
    ...
    showPlayButton = false
    if (is_opend === false && element.target.id !== 'videorepute-close' ||is_opend === false ) {
      showPlayButton = false
...
<main>
  <div
    ...
    {show_widget ? '' : 'hide'}"
    style="transform:{staticWidget ? staticWidgetResize : widgetResize};transform-origin:{leftSize=== "20px" && "bottom left 0px"};transform-origin:{rightSize=== "20px" && "bottom right 0px"};bottom:{is_opend && window.innerWidth <= 481 && '0'};bottom:{!is_opend && window.innerWidth <= 481 && '35px'};border-radius:20px;z-index:99;left:{leftSize === '20px' && window.innerWidth <= 481 && !is_opend ? '-220px' : leftSize === '20px' && window.innerWidth <= 481 && is_opend && !staticWidget ? '0' : leftSize};right:{rightSize};height:{staticWidget && is_opend && '400px'};height:{staticWidget && !is_opend && '64px'};width:{staticWidget && is_opend && '255px'};border-radius:{staticWidget && is_opend && '14px'};width:{staticWidget && window.innerWidth <=481 && is_opend && '100%'};z-index:{staticWidget && "1000"};bottom:{staticWidget && window.innerWidth <= 481 && !is_opend && "49px"};left:{staticWidget && window.innerWidth <=481 && !is_opend && "-124px"};left:{staticWidget && window.innerWidth <=481 && is_opend && "0"};left:{staticWidget && window.innerWidth > 482 && !is_opend && rightSize === '20px' && ""};width:{staticWidget && window.innerWidth > 482 && !is_opend && leftSize === '20px' && "351px"};width:{staticWidget && window.innerWidth > 482 && !is_opend && rightSize === '20px' && "255px"};left:{staticWidget && window.innerWidth > 482 && is_opend && rightSize === '20px' && ""};left:{staticWidget && desktopStatic &&  window.innerWidth <= 481 && !is_opend && rightSize==="20px" && "-28px"};left:{staticWidget &&  window.innerWidth <= 481 && !is_opend && rightSize==="20px" && "-28px"};left:{staticWidget &&  window.innerWidth <= 481 && !is_opend && rightSize==="20px" && "0"};">
    
    <!-- close mode -->
    <CloseWidgetIndex {desktopStatic} {is_opend} {staticTitle} {staticWidget} on:handleOpenVideoRepute={handleOpenVideoRepute} on:handleCloseText={handleCloseText} {rightSize} {gif_src} {textColor} {bgColor} {planType} {leftSize} {widgetInfo} {showText} />
        
      <!-- open mode -->
    <OpenWidgetIndex {currentScroll} {style} on:scrollFunction={scrollFunction}  on:handleReplay={handleReplay} {desktopStatic} {staticWidget} {replay} {selectShowVideo} {marginLeft} on:handleMute={handleMute} on:handleNext={handleNext} on:handleSpeedVideo={handleSpeedVideo} on:handleCloseSwipeText={handleCloseSwipeText} on:handleCloseVideoRepute={handleCloseVideoRepute} on:handlePlayButton={handlePlayButton} on:handlePrevious={handlePrevious} on:loadVideo={loadVideo} {iconColor} {current} {poster} {showTooltip} {timerWidth} {background} {currentMinutes} {currentSeconds} {speedOfVideo} {firstHeight} {showPlayButton} {rangeValue} {is_opend} {rightSize} {leftSize} {showSwipeText} {has_previous} {total} {has_next} {planType} />
  </div>
</main>
